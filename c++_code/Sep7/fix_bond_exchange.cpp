...
FixBondExchange::FixBondExchange(LAMMPS *lmp, int narg, char **arg) :
  ...
  distsq(nullptr), nextpartner(nullptr), nextdistsq(nullptr), nextbond(nullptr)
{
  ...
  nextbond = nullptr;
  FixBondExchange::grow_arrays(atom->nmax);
...
void FixBondExchange::setup(int /*vflag*/)
{
  int i, n, i1, i2;
  int bondtype;
...
void FixBondExchange::post_integrate()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    itype = type[i];
    if (nextbond[i]) inext = atom->map(nextbond[i]);
    else inext = 0;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (nextbond[j]) jnext = atom->map(nextbond[j]);
      else jnext = 0;
      ...
      /*if (inext && jnext) {
        delx = x[inext][0] - x[jnext][0];
        dely = x[inext][1] - x[jnext][1];
        delz = x[inext][2] - x[jnext][2];
        nextrsq = delx*delx + dely*dely + delz*delz;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    // exchange bond
    /*************************************************************/
    if ((possible == 1) && (exchange2 == 1) && (tag[i] < tag[j]) && ((tag[i] < tag[jnext]))) {
      foursome++;
      accept = 0;
      ...
      for (ibond = 0; ibond < num_bond[i]; ibond++)
        if (bond_atom[i][ibond] == inexttag) bond_atom[i][ibond] = jtag;
...
void FixBondExchange::unpack_forward_comm(int n, int first, double *buf)
{
  ...
  if (commflag == 1) {
    for (i = first; i < last; i++)
      nextbond[i] = (tagint) ubuf(buf[m++]).i;
  } else if (commflag == 2) {
    for (i = first; i < last; i++) {
...
void FixBondExchange::unpack_reverse_comm(int n, int *list, double *buf)
{
  ...
  if (commflag == 1) {
    for (i = 0; i < n; i++) {
      j = list[i];
      nextbond[j] += (tagint) ubuf(buf[m++]).i;
    }
  }
  else {
    for (i = 0; i < n; i++) {
      ...
      if((buf[m+1] < distsq[j]) && (buf[m+3] < nextdistsq[j])){
        partner[j] = (tagint) ubuf(buf[m++]).i;
        distsq[j] = buf[m++];
        nextpartner[j] = (tagint) ubuf(buf[m++]).i;
        nextdistsq[j] = buf[m++];
      }
      /*else if(buf[m+1] < distsq[j]){
        partner[j] = (tagint) ubuf(buf[m++]).i;
        distsq[j] = buf[m++];
        nextpartner[j] = (tagint) ubuf(buf[m++]).i;
        nextdistsq[j] = buf[m++];
...
void FixBondExchange::grow_arrays(int nmax)
{
  memory->grow(nextbond,nmax,"bond/create:nextbond");
}
...
void FixBondExchange::copy_arrays(int i, int j, int /*delflag*/)
{
  nextbond[j] = nextbond[i];
}
...
int FixBondExchange::pack_exchange(int i, double *buf)
{
  buf[0] = nextbond[i];
  return 1;
}
...
int FixBondExchange::unpack_exchange(int nlocal, double *buf)
{
  nextbond[nlocal] = static_cast<tagint> (buf[0]);
  return 1;
}
