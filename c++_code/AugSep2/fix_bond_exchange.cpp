...
void FixBondExchange::post_integrate()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jndex = atom->map(tag[j]);
      possible1 = 0;
      for (n = 0; n < nspecial[jndex][0]; n++){
        jnext = atom->map(special[jndex][n]);
        if (!(mask[jnext] & groupbit)) continue;
        if (jnext == i) continue;
        //if (jnext >= nlocal || jnext < 0) continue;
        if (type[jnext] != jtype){
          possible1 = 1;
          jnext = atom->map(tag[jnext]);
          break;
        }
      }
      ...
      if ((possible1==0 && possible==1)|| (possible1==1 && possible==0)) {
        if (rsq < distsq[i]) {
          partner[i] = tag[j];
          distsq[i] = rsq;
          if(possible==1)
      ...
      else if (possible1==1 && possible==1){
...
void FixBondExchange::unpack_reverse_comm(int n, int *list, double *buf)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if(buf[m+1] < distsq[j]){
      ...
      distsq[j] = buf[m++];
      inextpartner = (tagint) ubuf(buf[m]).i;
      if (inextpartner) {
        nextpartner[j] = (tagint) ubuf(buf[m++]).i;
        if (buf[m] < nextdistsq[j])
          nextdistsq[j] = buf[m++];
        else m += 1;
      } else m += 2;
    } else m += 4;
