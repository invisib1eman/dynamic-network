...
FixBondExchange::FixBondExchange(LAMMPS *lmp, int narg, char **arg) :
  ...
  next_partner(nullptr), next_finalpartner(nullptr)
{
...
FixBondExchange::~FixBondExchange()
{
  ...
  memory->destroy(next_partner);
  memory->destroy(finalpartner);
  memory->destroy(next_finalpartner);
...
{
  int i,j,k,m,n,ii,jj,inum,jnum,itype,jtype,n1,n2,n3,possible, possible1;
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(next_partner,nmax,"bond/exchange:next_partner");
    memory->create(finalpartner,nmax,"bond/exchange:finalpartner");
    memory->create(next_finalpartner,nmax,"bond/exchange:finalpartner");
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (m = 0; m < nspecial[i][0]; m++){
      ...
      if (type[inext] != itype && (mask[inext] & groupbit)){
        possible = 1;
        break;
      }
    }
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    /***************************************************/
    inext = atom->map(next_partner[j]);
    if (inext >= nlocal || inext < 0) continue;

    delta = energy_barrier;
    delta += pair_eng(i,inext) - pair_eng(i,j);
    delta += bond_eng(btype,j,i) - bond_eng(btype,i,inext);
    if (delta >= 0.0) {
      factor = exp(-delta/force->boltz/t_current);
      if (tag[i] < tag[j]) {
        if (probability[i] >= factor) continue;
      } else {
        if (probability[j] >= factor) continue;
      }
    }
    /*****************************************************/
    
    for (m = 0; m < num_bond[inext]; m++) {
      if (bond_atom[inext][m] == tag[i]) {
        for (k = m; k < num_bond[inext]-1; k++) {
    ...
    for (m = 0; m < nspecial[i][0]; m++)
      if (special[i][m] == tag[inext]) special[i][m] = tag[j];
  ...
  // create list of exchaged bonds that influence my owned atoms
  ...
  for (i = 0; i < nall; i++) {
    ...
    if (j < 0 || tag[i] < tag[j]) {
      if (nexchange == maxexchange) {
        maxexchange += DELTA;
        memory->grow(exchanged,maxexchange,2,"bond/exchange:exchaged");
      }
...
/* ----------------------------------------------------------------------
   double loop over my atoms and exchaged bonds
...
void FixBondExchange::update_topology()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (influenced) {
      ...
      if (angleflag) exchange_angles(i, id1, id2);
      if (dihedralflag) exchange_dihedrals(i, id1, id2);
      if (improperflag) exchange_impropers(i, id1, id2);
    }
  }
...
/* ----------------------------------------------------------------------
   exchange any angles owned by atom M that include atom IDs 1 and 2
   angle is exchaged if ID1-ID2 is one of 2 bonds in angle (I-J,J-K)
------------------------------------------------------------------------- */

void FixBondExchange::exchange_angles(int m, tagint id1, tagint id2)
{
  int j,found;
...
/* ----------------------------------------------------------------------
   exchange any dihedrals owned by atom M that include atom IDs 1 and 2
   dihedral is exchaged if ID1-ID2 is one of 3 bonds in dihedral (I-J,J-K.K-L)
------------------------------------------------------------------------- */

void FixBondExchange::exchange_dihedrals(int m, tagint id1, tagint id2)
{
  int j,found;
...
/* ----------------------------------------------------------------------
   exchange any impropers owned by atom M that include atom IDs 1 and 2
   improper is exchaged if ID1-ID2 is one of 3 bonds in improper (I-J,I-K,I-L)
------------------------------------------------------------------------- */

void FixBondExchange::exchange_impropers(int m, tagint id1, tagint id2)
{
  int j,found;
                                    ...
                                    int /*pbc_flag*/, int * /*pbc*/)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    buf[m++] = ubuf(next_finalpartner[j]).d;
    ns = nspecial[j][0];
    buf[m++] = ubuf(ns).d;
    for (k = 0; k < ns; k++)
...
void FixBondExchange::unpack_forward_comm(int n, int first, double *buf)
{
  ...
  if (commflag == 1) {
    ...
    for (i = first; i < last; i++) {
      ...
      next_partner[i] = (tagint) ubuf(buf[m++]).i;
      probability[i] = buf[m++];
    }
    ...
    for (i = first; i < last; i++) {
      ...
      next_finalpartner[i] = (tagint) ubuf(buf[m++]).i;
      ns = (int) ubuf(buf[m++]).i;
      nspecial[i][0] = ns;
      for (j = 0; j < ns; j++)
...
void FixBondExchange::unpack_reverse_comm(int n, int *list, double *buf)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (buf[m+1] > distsq[j]) {
      ...
      next_partner[j] = (tagint) ubuf(buf[m++]).i;
      distsq[j] = buf[m++];
    } else m += 3;
  }
}
...
{
  if (n == 0) return (double) exchangecount;
  return (double) exchangecounttotal;
}
